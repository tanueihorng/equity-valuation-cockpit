...
# ---- MARKET ----
row = grp("MARKET", row)
r_price   = row; row = put(row, "Current price ($/sh)", 185.00, NF_USD, "Live market price per share")
r_shares  = row; row = put(row, "Shares outstanding (M)", 1000, NF_NUM, "Diluted shares, millions")
r_hi52    = row; row = put(row, "52-week high ($)", 230, NF_USD, "52w high")
r_lo52    = row; row = put(row, "52-week low ($)", 140, NF_USD, "52w low")
r_swhi    = row; row = put(row, "Swing high ($)", 210, NF_USD, "Recent swing high for Fib")
r_swlo    = row; row = put(row, "Swing low ($)", 150, NF_USD, "Recent swing low for Fib")
r_ma50    = row; row = put(row, "50-day MA ($)", 182, NF_USD, "50DMA")
r_ma200   = row; row = put(row, "200-day MA ($)", 176, NF_USD, "200DMA")
r_rsi     = row; row = put(row, "RSI(14)", 55, NF_NUM1, "Relative strength index")
r_beta    = row; row = put(row, "Beta", 1.15, "0.00", "Levered equity beta")
row += 1

# ---- INCOME & CF (TTM) ----
row = grp("INCOME & CASH FLOW (TTM)", row)
r_rev     = row; row = put(row, "Revenue ($M)", 80000, NF_NUM, "TTM revenue, millions")
r_ebitm   = row; row = put(row, "EBIT margin", 0.28, NF_PCT, "Operating margin")
r_tax     = row; row = put(row, "Tax rate", 0.21, NF_PCT, "Effective / marginal tax")
r_da      = row; row = put(row, "D&A (% of revenue)", 0.04, NF_PCT, "Depreciation & amortization")
r_capex   = row; row = put(row, "CapEx (% of revenue)", 0.05, NF_PCT, "Capital expenditure")
r_nwc     = row; row = put(row, "Change in NWC (% of dRev)", 0.12, NF_PCT, "Working-cap investment on YoY rev increase")
r_intexp  = row; row = put(row, "Interest expense ($M)", 1000, NF_NUM, "TTM interest expense")
r_ni      = row; row = put(row, "Net income ($M)", 16900, NF_NUM, "TTM net income")
row += 1

# ---- BALANCE SHEET ----
row = grp("BALANCE SHEET", row)
r_debt    = row; row = put(row, "Total debt ($M)", 20000, NF_NUM, "Short + long term debt")
r_cash    = row; row = put(row, "Cash & equivalents ($M)", 15000, NF_NUM, "Cash + ST investments")
r_bve     = row; row = put(row, "Book value of equity ($M)", 50000, NF_NUM, "Total shareholders equity")
r_ta      = row; row = put(row, "Total assets ($M)", 95000, NF_NUM, "Total assets")
r_tca     = row; row = put(row, "Total current assets ($M)", 38000, NF_NUM, "Current assets")
r_tcl     = row; row = put(row, "Total current liabilities ($M)", 22000, NF_NUM, "Current liabilities")
r_re      = row; row = put(row, "Retained earnings ($M)", 30000, NF_NUM, "Retained earnings")
r_tl      = row; row = put(row, "Total liabilities ($M)", 45000, NF_NUM, "Total liabilities")
row += 1

# ---- GROWTH ----
row = grp("GROWTH", row)
r_g1 = row; row = put(row, "Revenue growth yr 1", 0.12, NF_PCT, "Explicit stage growth")
r_g2 = row; row = put(row, "Revenue growth yr 2", 0.11, NF_PCT, "")
r_g3 = row; row = put(row, "Revenue growth yr 3", 0.10, NF_PCT, "")
r_g4 = row; row = put(row, "Revenue growth yr 4", 0.09, NF_PCT, "")
r_g5 = row; row = put(row, "Revenue growth yr 5", 0.08, NF_PCT, "")
...
r_ppe   = row; row = put(row, "Peer P/E", 16.0, NF_MULT, "Trailing P/E median")
r_pfpe  = row; row = put(row, "Peer forward P/E", 14.0, NF_MULT, "Forward P/E median")
r_peve  = row; row = put(row, "Peer EV/EBITDA", 13.0, NF_MULT, "EV/EBITDA median")
r_pevs  = row; row = put(row, "Peer EV/Sales", 4.0, NF_MULT, "EV/Sales median")
r_ppb   = row; row = put(row, "Peer P/B", 4.5, NF_MULT, "Price/Book median")
r_ppfcf = row; row = put(row, "Peer P/FCF", 20.0, NF_MULT, "Price/FCF median")
row += 1

# ---- SCENARIO ----
row = grp("SCENARIO DELTAS", row)
r_exit_base = row; row = put(row, "Exit EV/EBITDA - BASE", 13.0, NF_MULT, "Base exit multiple")
r_exit_bear = row; row = put(row, "Exit EV/EBITDA - BEAR", 10.0, NF_MULT, "Bear exit multiple")
r_exit_bull = row; row = put(row, "Exit EV/EBITDA - BULL", 16.0, NF_MULT, "Bull exit multiple")
...
r_pe_lo = row; row = put(row, "P/E - 10y low", 9.0, NF_MULT, "Lowest ~10y P/E")
r_pe_md = row; row = put(row, "P/E - 10y median", 14.0, NF_MULT, "Median ~10y P/E")
r_pe_hi = row; row = put(row, "P/E - 10y high", 22.0, NF_MULT, "Highest ~10y P/E")
r_ev_lo = row; row = put(row, "EV/EBITDA - 10y low", 7.0, NF_MULT, "Lowest ~10y EV/EBITDA")
r_ev_md = row; row = put(row, "EV/EBITDA - 10y median", 11.0, NF_MULT, "Median ~10y EV/EBITDA")
r_ev_hi = row; row = put(row, "EV/EBITDA - 10y high", 16.0, NF_MULT, "Highest ~10y EV/EBITDA")
row += 1
...
# Save
OUT = "Equity_Valuation_Cockpit.xlsx"
wb.save(OUT)
print("SAVED:", OUT)
